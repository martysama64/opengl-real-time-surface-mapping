...
glm::vec2 bumpTexelSize(1.0f / 512.0f, 1.0f / 512.0f);
float bumpStrength = 3.0f;
...
void createDefaultFlatBumpTexture() {
    if (bumpTexture) {
    ...
    bumpTexelSize = glm::vec2(1.0f, 1.0f);
}

void loadBump(const std::string& path) {
    sf::Image img;
    if (!img.loadFromFile(path)) {
        std::cerr << "Failed to load bump texture: " << path << std::endl;
        return;
    }
...
void main() {
    ...
    if (useBump) {
        float hL = texture(bumpMap, TexCoord + vec2(-bumpTexelSize.x, 0.0)).r;
        float hR = texture(bumpMap, TexCoord + vec2( bumpTexelSize.x, 0.0)).r;
        float hD = texture(bumpMap, TexCoord + vec2(0.0, -bumpTexelSize.y)).r;
        float hU = texture(bumpMap, TexCoord + vec2(0.0,  bumpTexelSize.y)).r;

        float dx = hL - hR;
        float dy = hD - hU;

        vec3 localNormal = normalize(vec3(dx * bumpStrength, dy * bumpStrength, 1.0));
...
void draw(const sf::Window& window) {
    ...
    glUniform1i(glGetUniformLocation(Program, "useBump"), useBump ? 1 : 0);
    glUniform3f(glGetUniformLocation(Program, "objectColor"), sphereColor.x, sphereColor.y, sphereColor.z);
    glUniform2f(glGetUniformLocation(Program, "bumpTexelSize"), bumpTexelSize.x, bumpTexelSize.y);
    glUniform1f(glGetUniformLocation(Program, "bumpStrength"), bumpStrength);

    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, bumpTexture);
    glUniform1i(glGetUniformLocation(Program, "bumpMap"), 0);
...
int main() {
    sf::Window window(sf::VideoMode({ 1024, 768 }), "Bump Mapping", sf::Style::Default);
